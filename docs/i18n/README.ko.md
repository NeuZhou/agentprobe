...
**그런데 AI 에이전트는?** `console.log`와 눈으로 확인하는 게 전부인가요? 에이전트는 도구를 선택하고, 실패를 처리하고, 사용자 데이터를 다루며, 자율적으로 응답을 생성합니다. 프롬프트 하나 잘못되면 PII 유출. 도구 호출 하나 놓치면 워크플로우가 조용히 망가짐.

AgentProbe로 YAML 또는 TypeScript로 테스트를 작성하고, 텍스트 출력이 아닌 Tool Call(도구 호출)을 어설션하세요. 카오스를 주입하고, 리그레션을 사용자보다 먼저 잡아냅니다.
...
**에이전트가 프로덕션 데이터를 다룬다면, 제대로 된 테스트가 필요합니다. 프롬프트가 아니라 동작을 테스트하세요.**
